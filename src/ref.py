...
from polars import DataFrame, Int8, Utf8, col, concat, lit, when
...
class Borrowing:
    ...
    def __init__(self, data, id):
        ...
        self.disbursements_consol = self.process_disbursements(
            data["disbursements"], True
        )
        self.location: str = (
            list(data["disbursements"].values())[0].select("Location").item(0, 0)
        )
        self.cap_date = (
            list(data["disbursements"].values())[0]
            .select("Capitalization Date")
            .item(0, 0)
        )
        self.schedule = self.compute_schedule(self.payments, self.disbursements)
        self.consol_schedule = self.compute_schedule(
            self.payments, self.disbursements_consol
        )
        self.id = id
    ...
    def group_schedule(daily_schedule_df: DataFrame) -> DataFrame:
        ...
        if "IRR" in daily_schedule_df.columns:  # Standalone
            grouped = (
                daily_schedule_df.with_columns(
                    month.alias("Month"),
                    date.alias("From"),
                    date.alias("To"),
                    (
            ...
            grouped = grouped.with_columns(
                col("IRR").map_elements(get_map, float).alias("Interest Rate")
            )
            grouped = grouped.with_columns(
                (col("IRR", "Interest Rate") * 100).cast(Utf8) + lit("%")
            )
        else:  # Consol
            grouped = (
                daily_schedule_df.with_columns(
        ...
        amortmap = (
            grouped.group_by("Month")
            .agg(col("Amortization", "Days").sum())
            .with_columns(
                (col("Amortization") / col("Days")).alias("DailyAmortization")
            )
            .select("Month", "DailyAmortization")
        )
        grouped = (
            grouped.join(amortmap, on="Month")
            .with_columns(
                (col("DailyAmortization") * col("Days")).alias("Amortization")
            )
            .drop("DailyAmortization")
            .with_columns(
                col("Amortization")
                .cum_sum(reverse=True)
                .alias("Opening Unamortized Transaction Costs"),
                (col("Amortization").cum_sum(reverse=True) - col("Amortization")).alias(
                    "Closing Unamortized Transaction Costs"
                ),
            )
        )
        col_order = [
            ...
            for i in [
                ...
                "Principal Paid",
                "Interest Paid",
                "Opening Unamortized Transaction Costs",
                "Amortization",
                "Closing Unamortized Transaction Costs",
                "Interest Expensed",
                "Interest Capitalized",
    ...
    def to_dataframe(self, consol: bool = False) -> DataFrame:
        ...
        df = df.with_columns(
            lit(self.location).alias("Location"), lit(self.id).alias("id")
        )

        try:
            if self.cap_date not in ["", None]:
                df = df.with_columns(
                    when(col("Date") <= self.cap_date)
                    .then(col("EIR Interest"))
                    .otherwise(0)
                    .alias("Interest Capitalized"),
                    when(col("Date") > self.cap_date)
                    .then(col("EIR Interest"))
                    .otherwise(0)
                    .alias("Interest Expensed"),
                )
    ...
    def process_payments(
        payments_data: dict[DataFrame],
    ) -> dict[tuple[ndarray[float64 | datetime64]]]:
        data = {}
        for key in payments_data.keys():
            ...
            payment_dates: ndarray[datetime64] = (
                df.select(date_col).to_numpy().squeeze()
            )
            amendment_dates: ndarray[datetime64] = (
                df.select("Amendment").to_numpy().squeeze()
            )
    ...
    def process_disbursements(
        disbursements_data: dict[DataFrame], consol: bool
    ) -> dict[tuple[ndarray[float64 | datetime64]]]:
        data = {}
        for key in disbursements_data.keys():
            df = disbursements_data[key]
            loan_amount: ndarray[float64] = (
                df.select(loan_amount_col).to_numpy().squeeze()
            )
            cg_gst: ndarray[float64] = df.select(cg_gst_col).to_numpy().squeeze()
            loan_expenses: ndarray[float64] = (
                df.select(loan_exps_col).to_numpy().squeeze()
            )
    ...
    def compute_schedule(
        payments: dict[tuple[ndarray[float64 | datetime64]]],
        disbursements: dict[tuple[ndarray[float64 | datetime64]]],
    ) -> tuple[ndarray[float64, datetime64]]:
        def get_date_series(payment_dates, disbursement_dates):
            ...
            date_series = arange(
                start_date, end_date + timedelta64(1, "D"), timedelta64(1, "D")
            )
            return date_series

        def padded_cashflows(
            date_series: ndarray[datetime64],
            payment_dates: ndarray[datetime64],
            disbursement_dates: ndarray[datetime64],
            payments: ndarray[float64],
            disbursements: ndarray[float64],
            int_rate: ndarray[float64],
            int_paid: ndarray[float64],
        ) -> tuple[ndarray]:
            ...
            disbursement_indices = arange(len(date_series))[
                isin(date_series, disbursement_dates)
            ]
        ...
        def compute_daily_schedule(
            ...
            return (
                date_series,
                int_rate_padded,
                int_paid_padded.round(),
                opbal.round(),
                disbursements_padded.round(),
                interest.round(),
                payments_padded.round(),
                clbal.round(),
                __irr__,
            )
        ...
        for i in range(len(payments)):
            ...
            if i > 0:
                ...
                if (amendment_date > date_series.max()) or (
                    amendment_date < date_series.min()
                ):
                    raise ValueError(
                        "Current start date outside the daterange, please rectify the input files"
                    )
                ...
                try:
                    ...
                    if amendment_date in disbursements_dates:
                        ...
                        disbursements_dates = np_concat(
                            array([amendment_date]), disbursements_dates
                        )
                        disbursements_amounts = np_concat(
                            array([opbal_orig[-1]]), disbursements_dates
                        )
                except KeyError:
                    disb_on_amend_date = 0
                ...
                int_rate_padded = np_concat(
                    (int_rate_padded_orig[:-1], int_rate_padded)
                )
                int_paid_padded = np_concat(
                    (int_paid_padded_orig[:-1], int_paid_padded)
                )
                opbal = np_concat((opbal_orig, opbal[1:]))
                disbursements_padded_orig[-1] = disb_on_amend_date
                disbursements_padded = np_concat(
                    (disbursements_padded_orig, disbursements_padded[1:])
                )
                interest = np_concat((interest_orig[:-1], interest))
                payments_padded = np_concat(
                    (payments_padded_orig[:-1], payments_padded)
                )
    ...
    def consol(cls) -> DataFrame:
        dfs = [
            borrowing.to_dataframe(consol=True).with_columns(
                lit(borrowing.location).alias("Location")
            )
            for borrowing in Borrowing.borrowing_list
        ]
        dfs = concat(dfs)
        dfs = dfs.group_by(["Location", "Date"]).agg(
            col(
                "Interest Paid",
                "Opening Balance",
                "Disbursements",
    ...
    def journal_entry(self, consol: bool, month: str):
        ...
        grouped_schedule = (
            grouped_schedule.filter(col("Month") == month)
            .group_by("Month")
            .agg(
                col(
                    "EIR Interest",
        ...
        entry = DataFrame(
            [
                ["EIR", eir, "", f"EIR of {eir} accrued against loan {self.id}"],
                [
                    "To Borrowings",
                    "",
                    eir,
                    f"EIR of {eir} accrued against loan {self.id}",
                ],
                [
                    "Borrowings",
                    payment,
                    "",
                    f"Payment of {payment} made against loan {self.id}",
                ],
                [
                    "To Bank",
                    "",
                ...
                [
                    ...
                    expense,
                    "",
                    "EIR interest booked to profit and loss account",
                ],
                [
                    "Eligible Asset",
                    capitalise,
                    "",
                    "EIR interest capitalised against eligible asset",
                ],
                [
                    "EIR",
                    "",
                    eir,
                    f"EIR interest booked to profit and loss account to the extent of {expense} and capitalized to the extent of {capitalise}",
                ],
            ],
            schema=["Particulars", "Debit", "Credit", "Narration"],
            orient="row",
        )
        return entry
